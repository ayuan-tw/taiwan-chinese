...
function audioButton(text,label="🔊 音声"){
  if(!text)return "";
  const listen=`<button class="secondary small audio-btn" onclick="speakEncoded('${speechTextAttr(text)}')">${label}</button>`;
  const practice=typeof window.speechPracticeButton==="function"?window.speechPracticeButton(text):"";
